...
| `claude` | 対話セッション開始 |
| `claude "質問"` | ワンショット実行 |
| `claude -c` | 同じディレクトリの直前セッションを再開 |
| `claude --resume` | 過去のセッション一覧から選んで再開 |
| `/clear` | 会話をリセット（タスクごとに実行が基本） |
| `/compact` | 会話を要約して圧縮（同タスク継続時。`/compact 方針だけ残して` と指示も可） |
| `/resume` | 対話中にセッション一覧を開いて切り替え |
| `/rewind` | 会話とファイル変更をチェックポイントに巻き戻す |
| `Esc` | 生成を中断 |
| `Esc` ×2 | Rewind メニューを開く（入力欄が空のとき） |
...
| 少し前に戻ってやり直したい | ダブルEsc / `/rewind` |
| 中断したタスクを続きから | `claude -c` / `claude --resume` |
| 繰り返し使う手順書 | skill |
| 絶対に強制したい処理・禁止 | hooks |
