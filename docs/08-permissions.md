# ミッション08: permissions とセキュリティ — AIに渡す鍵を管理する
...
ミッション07で hooks を書いた `.claude/settings.json` には、もう一つの主役がいます。**permissions**——Claude Code が「何をしてよいか」のルールです。
...
重要なのは、**このルールを強制するのはモデルではなく Claude Code 本体**だということです。CLAUDE.md に「.env を読まないで」と書くのはモデルへのお願い（確率的）ですが、permissions.deny は機構による遮断（決定的）です。ミッション07の教訓——**お願いは方針に、強制は機構に**——がそのままセキュリティに効いてきます。
...
`app/.claude/settings.json` に permissions を追記します（ミッション07の hooks がある人は、同じファイルに並べて書きます。マージ済みの完成形は [solutions/08-permissions/settings.json](../solutions/08-permissions/settings.json)）:
...
- **付かなかった人**: モデルは埋め込み指示をよく無視します。でも**毎回無視する保証はどこにもありません**。ミッション07で見た「モデルの自制は確率的」と同じ構図です。
...
ミッション07と同じ原則です。**お願いは方針に、強制は機構に**。「絶対」が付くものは permissions か hooks へ。
...
- チーム標準の settings.json（allow / deny / hooks）を設計する。ミッション07の hooks と合わせると「チームの防護壁」一式になる
...
→ 次: [ミッション09: MCP でツールを接続する](09-mcp.md)
